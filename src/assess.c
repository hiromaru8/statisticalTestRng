...
main(int argc, char *argv[])
{
	...
	// 1 回のイテレーションで検定するするビット長 n (=sequenceLength) を指定する。
	tp.n = atoi(argv[1]);

	// 検定に必要なパラメータを指定する。
	tp.blockFrequencyBlockLength 			= 128; // 128 ビットごとにブロック頻度検定を行う。
	tp.nonOverlappingTemplateBlockLength 	= 9;   // 9 ビットのテンプレートを使用して非重複テンプレートマッチング検定を行う。
	tp.overlappingTemplateBlockLength 		= 9;   // 9 ビットのテンプレートを使用して重複テンプレートマッチング検定を行う。
	tp.approximateEntropyBlockLength 		= 10;  // 10 ビットごとに近似エントロピー検定を行う。
	tp.serialBlockLength 					= 16;  // 16 ビットごとにシリアル検定を行う。
	tp.linearComplexitySequenceLength 		= 500; // 500 ビットごとに線形複雑さ検定を行う。
	tp.numOfBitStreams 						= 1;   // ビットストリームの数を指定する。

	// ユーザに対して、検定するビットストリームの生成方法を選択させる。
	option = generatorOptions(&streamFile);
	
	// ユーザに対して、実行する検定を選択させる。
	chooseTests(); 
	// ユーザに対して、検定に必要なパラメータを指定させる。
	fixParameters();
