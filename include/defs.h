...
typedef struct _testParameters {
	int		n;	// LENGTH OF BIT STREAM
	int		blockFrequencyBlockLength; // BLOCK LENGTH FOR BLOCK FREQUENCY TEST
	int		nonOverlappingTemplateBlockLength; // BLOCK LENGTH FOR NONOVERLAPPING TEMPLATE MATCHING TEST
	int		overlappingTemplateBlockLength; // BLOCK LENGTH FOR OVERLAPPING TEMPLATE MATCHING TEST
	int		serialBlockLength; // BLOCK LENGTH FOR SERIAL TEST
	int		linearComplexitySequenceLength; //	SEQUENCE LENGTH FOR LINEAR COMPLEXITY TEST
	int		approximateEntropyBlockLength; // BLOCK LENGTH FOR APPROXIMATE ENTROPY TEST
	int		numOfBitStreams; // NUMBER OF BIT STREAMS TO BE TESTED (FOR PARTITIONING PURPOSES)
} TP;
